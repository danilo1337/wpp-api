...
import br.com.danilo.dto.importacao.ImportacaoDTO;
...
public class ContatoNeg {
	...
	public void salvarArquivo(MultipartFile file) throws WppException, IOException {
		...
		BeanUtils.copyProperties(importacao, importacaoDTO);
		importacaoDTO.setUsuario(importacao.getUsuario().getId());
        ...
        HttpEntity<ImportacaoDTO> entity = new HttpEntity<>(importacaoDTO, headers);
	 ...
	 *
	 * O padrão esperado para o arquivo CSV é: - As colunas devem estar separadas
	 * por ponto e vírgula (;). - As colunas devem estar nessa ordem: cpf_cnpj,
	 * nome, telefone e observacao.
	 *
	 * O padrão esperado é: cpf_cnpj;nome;telefone;observacao
	 *
	 * @param file O arquivo CSV enviado como MultipartFile.
	 * @return true se o arquivo estiver no padrão esperado, caso contrário, retorna
	 *         false.
	 */
